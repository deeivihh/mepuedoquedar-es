...
function buildLegends(
    ...
    data: any,
    formatName?: (name: string) => string
): { label: string; extra?: string; percentage?: string }[] | null {
    if (isCategorical) {
...
function ChartLegendList({
    ...
    legends: { label: string; extra?: string; percentage?: string }[];
    isManyLegends: boolean;
}) {
    return (
        <div
            className={
...
function ChartHeader({
    ...
    return (
        <div className="flex justify-between w-full items-center">
            {title ? <h3 className="font-semibold text-title text-sm md:text-base">{title}</h3> : null}
...
function useBaseChart({ type = "line", data, title, formatName }: BaseChartProps) {
    const isPolar = type === "pie" || type === "donut";
    ...
    const { flatData, pieData, latestYear } = useMemo(
        () => prepareChartData(data, isCategorical, isPolar, isSeriesArray, title, formatName),
    ...
    const definition = useMemo(
        () => buildChartDefinition(type, pieData, flatData, isSeriesArray, data),
        [type, pieData, flatData, isSeriesArray, data]
    );

    const legends = useMemo(
        () => buildLegends(isCategorical, pieData, totalPieValue, isSeriesArray, data, formatName),
        [isCategorical, pieData, totalPieValue, isSeriesArray, data, formatName]
    );

    const hasLegends = Boolean(legends && legends.length > 0);
    const isManyLegends = Boolean(legends && legends.length > 2);
...
export default function BaseChart({ type = "line", data, title, height = 260, formatName }: BaseChartProps) {
    ...
    const { isPolar, definition, legends, latestYear, hasLegends, isManyLegends } = useBaseChart({
        type,
        data,
        title,
        formatName,
        ...
        <div className="flex flex-col gap-3 w-full text-title relative">
            ...
            <div className="w-full relative">
                <Chart
                    ...
                    height={height}
                    className="w-full"
                    renderTooltipBody={({ points }) => <ChartTooltipBody points={points} isPolar={isPolar} />}
                />
            </div>

            {hasLegends && showLegend && legends ? (
                <ChartLegendList legends={legends} isManyLegends={isManyLegends} />
            ) : null}
        </div>
    );
}
