...
function createLegends(
    ...
    pieData: any[],
    isSeriesArray: boolean,
    data: any[],
    formatName?: (name: string) => string
): LegendItem[] | null {
    if (isCategorical) {
        const total = pieData.reduce((acc: number, item: any) => acc + item.value, 0);
        return pieData.map((d: any) => {
            const pct = total > 0 ? ((d.value / total) * 100).toFixed(0) : undefined;
            return {
                label: d.label,
    ...
    if (isSeriesArray && data.length > 1) {
...
function ChartLegend({ legends }: { legends: LegendItem[] }) {
    const isManyLegends = legends.length > 2;

    return (
        <div
            className={`flex ${isManyLegends ? "flex-col sm:grid sm:grid-cols-2" : "flex-wrap items-center justify-center"} gap-x-4 gap-y-1.5 pt-2 border-t border-title/10 w-full text-xs`}
        >
...
function ChartTooltipBody({ points, isPolar }: { points: readonly any[]; isPolar: boolean }) {
    ...
    return (
        <div className="flex items-center gap-1.5 text-xs">
            ...
            {points.map((p) => {
                ...
                const pointKey = p.key ?? `${p.series ?? (p.datum as any)?.label ?? "val"}-${p.xValue}`;
                return (
...
export default function BaseChart({ type = "line", data, title, height = 260, formatName }: BaseChartProps) {
    ...
    const chartData = useMemo(() => {
        return parseChartData(type, data, title, formatName);
    }, [type, data, title, formatName]);

    const definition = useMemo(() => {
        if (chartData.isPolar) {
            return createPolarDefinition(type, chartData.pieData);
        }
        if (type === "column") {
            return createColumnDefinition(chartData.pieData);
        }
        const isSingle = !chartData.isSeriesArray || data.length === 1;
        return createCartesianDefinition(type, chartData.flatData, isSingle);
    }, [type, chartData, data]);

    const legends = useMemo(() => {
        return createLegends(chartData.isCategorical, chartData.pieData, chartData.isSeriesArray, data, formatName);
    ...
    return (
        <div className="flex flex-col gap-3 w-full text-title relative">
            <ChartHeader
                title={title}
                latestYear={chartData.latestYear}
            ...
            <div className="w-full relative">
                <Chart
                    ...
                    renderTooltipBody={({ points }) => (
                        <ChartTooltipBody points={points} isPolar={chartData.isPolar} />
                    )}
                />
            </div>
