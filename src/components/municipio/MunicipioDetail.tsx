...
function useMunicipioDetail(cod: string, locationParams: string, ready: boolean, preferences: any, isDefault: boolean) {
    ...
    const fetchAllData = useCallback(async () => {
        ...
        try {
            ...
            const wikiPromise = getMunicipioWikipedia(dbData.latitud, dbData.longitud, dbData.municipio, dbData.provincia).catch(() => null);
            const inePromise = dbData.cod_int ? fetchAllTables(TABLES, dbData.cod_int).catch(() => ({})) : null;
    ...
    useEffect(() => {
        ...
        const calculatedScores = calculateScores(data, multipliers, ineData);
...
function MunicipioError({ error, onRetry }: { error: string; onRetry: () => void }) {
    return (
...
export default function MunicipioDetail({ cod }: { cod: string }) {
    ...
    const {
        ...
        ineData,
        fetchAllData,
    } = useMunicipioDetail(cod, locationParams, ready, preferences, isDefault);

    if (isLoading) return <MunicipioLoading />;
    if (error) return <MunicipioError error={error} onRetry={fetchAllData} />;
    if (!data) return <MunicipioNotFound />;
